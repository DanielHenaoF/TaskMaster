...
const transporter = nodemailer.createTransport({
  service: "gmail",
  auth: {
    user: "Danielhf7777@gmail.com",
    pass: "bfes iilr snrn skhr",
  },
});
...
router.post("/request-reset", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error al solicitar recuperación de contraseña" });
  }
});
...
router.get("/reset-password/:token", (req, res) => {
  ...
  console.log(
    "Accediendo al formulario de restablecimiento de contraseña con token:",
    token
  );
...
router.post("/reset-password", async (req, res) => {
  ...
  try {
    ...
    res.redirect("https://taskmaster-flax-nu.vercel.app");
  } catch (error) {
...
router.put("/update", authenticateToken, async (req, res) => {
  ...
  const { username, email, phone, city, currentPassword, newPassword } =
    req.body;
