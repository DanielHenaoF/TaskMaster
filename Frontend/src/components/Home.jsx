...
const Home = () => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      const response = await fetch('https://taskmaster-cfkw.onrender.com/api/auth/getTasks', {
        method: 'GET',
        headers: {
  ...
  const handleCreateTask = async () => {
    if (newTask.title.trim()) {
      try {
        ...
        const response = await fetch('https://taskmaster-cfkw.onrender.com/api/auth/tasks', {
          method: 'POST',
          headers: {
  ...
  const handleEditTask = async () => {
    if (editTask && newTask.title.trim()) {
      try {
        ...
        const response = await fetch(`https://taskmaster-cfkw.onrender.com/api/auth/tasks/${editTask.id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
  ...
  const handleUpdateStatus = async (taskId, newStatus) => {
    try {
      ...
      const response = await fetch(`https://taskmaster-cfkw.onrender.com/api/auth/tasks/${taskId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
      ...
      <div className="mb-4 flex justify-center flex-wrap space-x-2">
        {['All', 'Pendiente', 'En Progreso', 'Completada'].map((status) => (
          <button
            ...
            className={`px-3 py-2 border text-gray-800 rounded-lg text-sm font-semibold ${filter === status ? 'bg-indigo-600 text-white' : ''
              }`}
          >
